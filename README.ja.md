...
| 記事を書く | 提供された素材を整理し、必要なら Web で補足して直接執筆する | 使える記事または Newsletter |
...
接続後は、原資料、テーマごとの一つの正本、完全な執筆事例、独立した書き出し、確認済みの作品、コンテンツ方向、継続テーマの状態を保存できます。通常の記事執筆で読むのは記事事例と書き出しだけです。テーマ知識、個人の文体、公開履歴を新しい原稿へ自動で持ち込まず、対応する成果を明示的に求めた場合だけ利用します。一度の公開結果が長期戦略や個人の文体を自動で書き換えることもありません。
...
この Skill は、素材理解、テーマ調査、概念説明、実践設計、コンテンツレビュー、記事、Newsletter を扱います。短文、Thread、GitHub プロジェクトの短い紹介や一覧、プロジェクト・製品の短い宣伝文、および画像、GIF、動画、音声、Podcast の制作、一般翻訳、広告運用、セールスページ、メール施策、ブランド全体やマーケティング全体の戦略、実際の公開は別の能力です。
