...
```powershell
python <private-knowledge-skill>/scripts/private_library.py init --root "D:\Knowledge\100x-learning"
```
...
```powershell
python <private-knowledge-skill>/scripts/private_library.py validate
...
```powershell
python <private-knowledge-skill>/scripts/private_library.py adopt --root "E:\Knowledge\Existing Library"
```
...
```powershell
python <private-knowledge-skill>/scripts/private_library.py configure-marktree --cli "D:\Tools\Marktree\marktree-cli.exe"
python <private-knowledge-skill>/scripts/marktree_integration.py status
```
...
普通写作只在准备读取现有案例和钩子时尝试运行 `show` 取得路径，不运行 `validate`。取得路径后只打开案例索引、索引指向的完整案例、钩子索引和索引指向的完整钩子，不读取库首页、主题知识、其它来源、项目、成果、作者声音、发布历史或内容策略。配置不存在、路径不可达或参考不可用时继续写作，不把内部状态告诉用户，也不临时初始化私人库。
