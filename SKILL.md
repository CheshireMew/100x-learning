...
3. **多份完整案例和开头钩子实际参与表达。** 正常写作优先从本地私人库读取多份彼此互补的参考写作案例和参考开头钩子，并打开相应完整原文；实际选入的参考必须进入成稿的表达，私人库或参考不可用时继续写作。
...
选择参考时，根据本次成品需要的表达作用，分别从活动案例索引和钩子索引选择三个不同的写作技巧分组，再从每个分组沿一个稳定编号链接打开完整原文。阅读全文后，某份参考质量不足、与当前表达作用无关、和其它参考重复同一种写法或正文不可用时，优先在原分组换读；原分组没有可用正文时，在承担相同或相邻表达作用的分组中换读。索引标签只负责确定候选分组，完整原文决定最终取舍；题材、行业、对象、具体事件或邀请机制相同都不是前提。
