...
3. **先让读者看懂，再分开选择写作部件。** 普通写作先从完整材料里选择一个目标读者无需预先读过来源、也能立即明白发生了什么或能得到什么的角度，再从 `references/writing-templates/` 选择钩子、正文和结尾。承接与连接只有成稿确实需要独立句段时才选；文章与 Newsletter 再为实际章节选择章节块。组件不预先绑定成整篇路线；最终列出各自的 ID 与名称，未使用的类别如实标明。
4. **模板管职责，完整例子帮助表达。** 组件的适用条件和必须事实决定能不能选，组件只说明实际使用的部分负责什么、怎样推进和在哪里停，不提供可以填空的成品句。私人库可用时先查看候选标题，再打开仍可能相关的完整真实例子进行比较，由模型决定实际模仿哪一份；没有相关例子就不模仿。例子可以帮助判断读者入口、背景交代、术语解释、信息密度、推进和停笔，人物、事实、观点和句子仍来自当前材料。
...
普通写作只从私人库读取当前组件需要的模仿例子，不读取作者声音、发布历史或同主题知识，文章和 Newsletter 也不例外。用户只要求“像我写”时，使用当前请求中的原话和用户直接提供的样稿；只有用户明确要求读取私人库中的既有声音或作品时，才把声音检索作为一项单独的明确结果。任何模仿例子都不会因为使用第一人称而自动成为用户声音。

单个 GitHub 项目、项目清单、项目或产品宣发、文章、Newsletter 和包含发布事实的内容可以读取对应专项说明。每份专项说明只帮助准备对应事实和边界，在准备材料后停止；结构只由用户明确要求或组件模板目录决定。

完整读取 `references/writing-templates/index.md` 和当前材料。用户没有指定受众时，默认写给对题目感兴趣的普通读者；不假定他读过来源、从事相关行业、会编程或熟悉其中专门说法。先比较材料中的结果、动作、异常、问题、场景、输入和数字，选出一个读者无需先解码命令、英文名称、专业术语或统计口径，也能明白发生了什么或能得到什么，并且后文能够兑现的角度。项目类型、内容形式和模板编号不能代替选角度。角度确定后再筛选一个钩子 `H`、一个正文推进 `P` 和一个结尾 `E`。钩子后确实需要独立承接句时才选 `C`，两个已写部分之间确实出现连接句段时才选 `B`；文章与 Newsletter 为每个实际章节选择一个 `S`。组件之间不使用预先绑定的路线、项目类型默认路线或推荐组合。

用户明确规定某一部分时，只把该部分记为对应的 `USER-H`、`USER-C`、`USER-P`、`USER-B`、`USER-E` 或 `USER-S`，其它部分仍按实际写法决定是否使用。用户只要求改字词、格式或等义措辞时保持现有结构，不重新选组件。

逐个确认当前材料具备所选组件的必须事实。某个组件条件不成立时只换同类组件；`H`、`P` 或 `E` 没有合格组件时不成文，只指出缺少的最小事实集合。`C`、`B` 没有被实际使用时不选，不用空组件补结构。不能让模仿例子补充本轮人物、事实、观点、情绪或行动入口。

私人库已经配置且可读、并且模仿可能有帮助时，先运行 `scripts/select_writing_examples.py candidates` 查看所选 `H`、`P`、`E` 和长文实际使用的 `S` 的全部活动候选标题。标题只用于缩小范围，不能据此直接押定模仿对象；对仍可能帮助当前读者入口或表达的候选运行 `scripts/select_writing_examples.py render`，完整读完后再决定实际模仿哪一份。可以为同一组件比较多份完整候选，不设固定阅读数量，也不为多读而堆参考；最终每个组件最多把一份实际采用的例子放入成文输入，也可以不采用。禁止由脚本按固定排序自动选例。`C` 和 `B` 不加载风格例子。私人库不可用或没有相关例子时省略相应模仿例子，仍把组件当作职责和推进关系使用。按 `references/content-writing.md` 把用户要求、准备后的来源材料、当前角度、实际使用的组件、实际采用的模仿例子，以及用户在当前请求中提供或明确要求取得的声音材料组成当前请求的成文输入。材料准备、选角度、选模、取例和成文在同一次回复中连续完成。

草稿完成后完整读一遍：不了解来源的目标读者能否从前两句明白对象、变化或结果以及为什么值得继续读；命令、数字和专门说法是否只在帮助理解时保留，并在第一次出现时接上自然中文；同一句若堆了两个以上目标读者不熟悉的命令、英文名或专业术语，先改成读者熟悉的动作、结果或关系，只有需要搜索和操作的原名才保留；钩子是否在后文兑现；正文是否以所选 `P` 为主要推进方向；实际使用的承接和连接是否必要；结尾是否在所选 `E` 的位置停住。用户没有明确要求辟谣或纠正常见误解时，短内容不采用“不是、并非或不只是某事，再用而是、却或真正转到重点”的纠偏结构；来源自己写了免责声明，只决定事实能说到哪里，不要求沿用它的否定句式，直接正面写材料目前能支持什么。再从最终文字反向确认组件：正文可以加入帮助读者理解主线的必要解释，不把这种解释误判成第二个 `P`；只有主体真的改用另一种推进方式时才删除、重写或重选。删掉为了凑齐组件产生的身份句、解释句、过渡句，以及重复、空泛总结和从模仿例子带入的题材内容；如果成稿能明显还原成带方括号的填空句，重新自然表达这一句。用户明确要求多个候选时，每个候选分别从材料选角度，再独立选择实际需要的组件和模仿例子；没有指定数量时只生成一个。
...
默认先交付可直接使用的完整成品，随后固定列出本次使用的模板：钩子 `H`、正文 `P` 和结尾 `E` 写实际 ID 与名称；承接 `C`、连接 `B`、章节 `S` 只有实际使用时写 ID 与名称，否则写“未使用”。用户明确要求多个候选时，每个候选各自列出组件；较长、结构复杂或需要继续修改的文字可以另存临时 Markdown 并提供可点击链接。

默认不展示内部材料、组件选择过程、模仿例子、检查过程或覆盖账本路径。用户明确要求查看依据或写作过程时，再补充实际使用的材料和例子来源；不展示隐藏推理，也不把内部检查清单混进正文。
...
个人写作记忆只提供经过确认的写作风格样本与发布历史；内容案例库和钩子库提供模板蒸馏语料，以及由正式取例工具列出、再经模型按当前角度显式选择的模仿例子。三者都不能替代当前对象的事实和明确要求。普通翻译、转写、格式清洗和普通计划走对应专门流程；广告投放、销售页与落地页、邮件营销序列、品牌全案、完整营销策略和实际发布不属于本 Skill 的文字写作结果。

## 交付前检查

- 成品直接回答用户要求；资料足够时已经停止。
- 用户指定的重点、材料关系、数量、语言、格式和权限保持不变。
- 成文输入使用 `references/content-writing.md` 中唯一输入格式，先包含完整材料和当前选择的具体角度，再包含一个 `H`、一个 `P`、一个 `E`，以及实际使用的 `C`、`B` 和长文章节 `S`；没有混入覆盖账本、专项说明或维护过程。
- 普通写作没有读取同主题知识、作者声音或发布历史；私人库可用时先看候选标题，可以完整比较多份仍可能相关的例子，正式成文时每个组件最多采用一份，没有相关例子时不采用；长期保存、发布和其它外部动作只在用户明确要求时执行。
- 成品事实都来自当前材料，模仿例子只影响对应部分的表达；开头使用了足够具体的细节并在后文兑现，没有为了凑组件补身份句或过渡句，结尾自然停在 `E` 负责的位置，成品后已经列出各类模板的实际使用情况。
