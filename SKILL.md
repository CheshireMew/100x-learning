...
3. **三个完整案例和三个完整钩子实际参与表达。** 正常写作优先从本地私人库读取三份彼此互补的参考写作案例和三份参考开头钩子，并打开相应完整原文。三份案例共同影响整篇表达，三份钩子共同影响开头及其进入正文的方式；成文直接模仿完整原文中的具体表达，不把六份参考转换成技巧摘要或拼接任务。私人库或参考不可用时继续写作。
...
选择参考时，根据本次成品需要的表达作用，分别从活动案例索引和钩子索引选择三个不同的写作技巧分组，再从每个分组沿一个稳定编号链接打开完整原文。最终选中的三份案例应当在整篇写法上彼此互补，三份钩子应当在开头及其承接上彼此互补，并且这些具体写法能够自然出现在同一篇成品里。能像自然中文原稿一样直接读，是参考质量的前提；主要靠翻译腔、通用夸张或悬念套话维持吸引力的候选，视为质量不足。阅读全文后，某份参考质量不足、与当前表达作用无关、和其它参考重复同一种写法或正文不可用时，优先在原分组换读；原分组没有可用正文时，在承担相同或相邻表达作用的分组中换读。索引标签只负责确定候选分组，完整原文决定最终取舍；题材、行业、对象、具体事件或邀请机制相同都不是前提。
