...
3. **分开选择每个写作部件。** 普通写作从 `references/writing-templates/` 分别选择一个钩子、承接、正文推进、连接和结尾；文章与 Newsletter 还为每个章节选择章节块。组件可以独立组合，不预先绑定成整篇路线；最终列出各自的 ID 与名称。
4. **模板管职责，真实例子帮助表达。** 组件的适用条件和必须事实决定能不能选，组件只说明该部分负责什么、怎样推进和在哪里停，不提供可以填空的成品句。私人库可用时，分别取得钩子、正文、结尾和长文章节的完整真实例子，让模型只模仿对应部分的节奏、取舍和推进；人物、事实、观点和句子仍来自当前材料。
...
完整读取 `references/writing-templates/index.md`，再按用户要求的成品目的和当前材料分别筛选钩子 `H`、承接 `C`、正文推进 `P`、连接 `B` 和结尾 `E`；文章与 Newsletter 还为每个章节筛选章节块 `S`。只打开各类合格候选文件，最终每类选择一个；连接没有必要时选择 `B01`，长文不同章节可以分别选一个 `S`。项目介绍用具体利益、可见结果和事实证据展开；操作演示再使用命令和输入输出组件。组件之间不使用预先绑定的路线或推荐组合。

用户明确规定某一部分时，只把该部分记为对应的 `USER-H`、`USER-C`、`USER-P`、`USER-B`、`USER-E` 或 `USER-S`，其它部分仍从各自目录选择。用户只要求改字词、格式或等义措辞时保持现有结构，不重新选组件。

逐个确认当前材料具备所选组件的必须事实。某个组件条件不成立时只换同类组件；任何必需类别都没有合格组件时不成文，只指出缺少的最小事实集合。不能让模仿例子补充本轮人物、事实、观点、情绪或行动入口。

私人库已经配置且可读时，运行 `scripts/select_writing_examples.py`，为本次选中的 `H`、`P`、`E` 分别取得一份完整真实例子；文章与 Newsletter 再为实际使用的每种 `S` 取得一份。`C` 和 `B` 只承担承接与连接关系，不另加载风格例子。私人库不可用时省略模仿例子，仍把组件当作职责和推进关系使用，不临时编造成品句式。按 `references/content-writing.md` 把用户要求、准备后的来源材料、分别选中的组件、可用的模仿例子，以及用户在当前请求中提供或明确要求取得的声音材料组成当前请求的成文输入。材料准备、选模、取例和成文在同一次回复中连续完成。

草稿完成后完整读一遍：钩子是否真的值得继续读并在后文兑现，正文是否按所选 `P` 自然推进，承接有没有重复开头，连接是否表达真实关系，结尾是否在所选 `E` 的位置停住。删掉重复、空泛总结和从模仿例子带入的题材内容；如果成稿能明显还原成带方括号的填空句，重新自然表达这一句。用户明确要求多个候选时，每个候选分别独立选择全套组件和模仿例子；没有指定数量时只生成一个。
...
个人写作记忆只提供经过确认的写作风格样本与发布历史；内容案例库和钩子库提供模板蒸馏语料，以及由正式取例工具为当前组件挑出的一份完整模仿例子。三者都不能替代当前对象的事实和明确要求。普通翻译、转写、格式清洗和普通计划走对应专门流程；广告投放、销售页与落地页、邮件营销序列、品牌全案、完整营销策略和实际发布不属于本 Skill 的文字写作结果。

## 交付前检查

- 成品直接回答用户要求；资料足够时已经停止。
- 用户指定的重点、材料关系、数量、语言、格式和权限保持不变。
- 成文输入使用 `references/content-writing.md` 中唯一输入格式，分别包含一个 `H`、`C`、`P`、`B`、`E`，长文每章一个 `S`；私人库可用时，钩子、正文、结尾和实际使用的章节各有一份独立完整模仿例子，没有混入覆盖账本、专项说明或维护过程。
- 普通写作没有读取同主题知识、作者声音或发布历史；私人库可用时只读取选中组件的模仿例子；长期保存、发布和其它外部动作只在用户明确要求时执行。
- 成品事实都来自当前材料，模仿例子只影响对应部分的表达；开头已经兑现，组件之间没有重复或虚构连接，结尾自然停在 `E` 负责的位置，成品后已经列出全部模板 ID 与名称。
