...
用于初始化新的私人知识库、接入已有知识库，以及在知识读取、写入和维护开始前取得唯一根目录。本文件只负责根目录和冷启动，不决定材料应该沉淀成什么。
...
没有给保存位置时只询问这一项。新库必须位于 Skill 目录之外。初始化器会建立 `Home.md`、知识、来源、项目、成果、长期领域、系统和归档目录，不根据空白库推测用户兴趣或长期目标。唯一根目录记录到当前用户的 `~/.100x-learning/config.json`。目标目录已经包含其它内容但没有私人库标识时停止，请用户确认是否接入现有知识库。
...
两条命令都应从本机配置重新找到同一个根目录。重复初始化已经有效的同一个库时保留已有文件内容，只补齐当前合同要求的缺失目录和 `Home.md`，不覆盖用户已经修改的文件。
...
用户明确启用时，Marktree 只负责工作区范围、保留原文、避免同时写入冲突、操作恢复和可选 Git；本 Skill 继续决定知识、来源、项目和成果分别该放在哪里。不要在 Marktree 内复制一套知识分类规则。

现有私人库可能包含早期版本建立的写作案例、钩子、写作输出或写作系统。接入、显示和验证当前知识库时忽略这些目录，不删除、不移动、不补齐，也不把其中内容用于当前任务。
