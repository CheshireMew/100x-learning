...
配置不存在、根目录不可达或库版本无效时，把这个实际状态直接交回主流程。普通材料理解可以继续使用当前输入，独立研究可以继续执行外部来源路线；材料完备短内容和发现式新写都需要私人案例与钩子，因此在任何外部材料发现之前立即停止：配置不存在时明确告诉用户“没有配置私人库”，其它情况准确说明私人库当前不可用，并请用户先初始化、接入或修复。本人现稿的字词、格式或等义压缩仍按局部修改路线完成。用户要求的结果必须保存到本机时，同样询问初始化或接入位置后停止。
