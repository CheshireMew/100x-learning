...
普通写作先使用版本控制内分开存放的组件模板目录。私人库已经配置且可读时，主流程可以运行 `scripts/select_writing_examples.py`，只取得本次钩子、正文、结尾和长文章节组件对应的完整模仿例子；不读取同主题知识、作者声音、发布历史或其它私人材料。私人库配置不存在或路径不可达时省略模仿例子，仍按组件职责直接成文。
