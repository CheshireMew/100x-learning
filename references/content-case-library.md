...
用于用户明确要求时保存、更新和维护完整案例。案例库不管理钩子；活动案例既是独立组件模板的蒸馏语料，也可以由正式取例工具列为钩子、正文、结尾或长文章节的模仿候选。普通写作先用标题缩小候选范围，再为同一组件完整比较仍可能相关的案例，正式成文时最多采用其中一份；工具不在钩子库与案例库之间自动排优先级。模仿时不把案例事实带入当前内容。
...
活动案例发生新增、修改或归档时，如果用户本次同时要求更新模板，必须重新阅读全文，并在逐来源覆盖账本中让该 `case_id` 只出现一次：指向它实际支持的 `H/C/P/B/E/S` 组件，或写明拒绝原因。索引标签不能自动生成覆盖映射。

## 通过标准

- 案例脚本、目录与索引只区分社交内容和文章；社交内容不再区分独立短帖与 Thread，且案例库不读取钩子库。
- 活动案例保存完整原文、稳定寻址字段和必要写作技巧，不保存题材检索字段，也不绕过正式取例工具整批进入普通成文输入。
- 归档内容不会进入活动索引。
