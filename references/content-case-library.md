...
用于用户明确要求时保存、更新和维护完整案例。案例库不管理钩子；活动案例既是独立组件模板的蒸馏语料，也可以由正式取例工具列为钩子、正文、结尾或长文章节的模仿候选。普通写作先按当前角度判断候选是否相关，再决定是否显式读取一份完整案例；工具不在钩子库与案例库之间自动排优先级。模仿时不把案例事实带入当前内容。
