...
Ordinary writing first chooses the strongest concrete angle from the complete material, then independently selects only the version-controlled hook, continuation, body, bridge, ending, and long-form section components it actually needs. The finished copy lists each component used and marks optional components that did not appear. When the private library is available, the example selector lists candidate titles first; the model then decides, by relevance to the current angle, whether to read at most one complete case for a component. It never auto-selects a case. Examples can guide expression but cannot supply facts. Ordinary writing does not read topic knowledge, personal voice, or publication history unless you explicitly request the corresponding task. A single publication outcome does not rewrite long-term strategy or personal voice.
