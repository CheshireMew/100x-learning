...
`100x-learning` は、オープンな [Agent Skills 仕様](https://agentskills.io/specification)に準拠した学習・記事執筆 Skill です。字幕、記事、リンク、テーマ、プロジェクト、下書き、公開後の反応を読み取り、求められた最終結果に合う方法を選びます。
...
- **字幕を整えて説明する**：`この自動字幕から広告と主題に無関係な挿入を除き、誤認識を直して、残る原文を言い換えずに記事のような全文へ整え、素材を説明して個人知識ライブラリへ保存してください。`
- **調査して判断する**：`X を調査し、私の目的には A と B のどちらが合うか比較し、まだ分からない点も残してください。`
- **説明して応用する**：`実際の場面で X を説明し、それを使って私が直面している Y を分析してください。`
- **レビューまたは記事を執筆する**：`事実、構成、AI らしい表現だけを確認し、書き直さないでください。` または `この素材を一般読者向けの記事にしてください。`
- **継続して学び、改善する**：`X を継続して学びます。今回は最も役立つ一課を終え、私の反応から次を決めてください。`
- **知識を残す**：`確認済みの結論を個人知識ライブラリに保存し、同じテーマの既存の正本を更新してください。`

Skill は指定された成果で停止します。レビューが勝手に書き直しへ進むことも、候補が記事へ変わることも、一度のテーマ選定が長期プロジェクトになることもありません。ファイル保存、メディア制作、アップロード、公開もそれぞれ独立した操作です。
...
| --- | --- | --- |
| 字幕・自動文字起こし | 明確な広告と主題外の挿入を除き、残る原文と実在する時刻を保ち、確認できる誤認識を修正して自然な段落へ整え、その後に素材を分析して知識として残す | 削除・修正の説明を含む完全な主題字幕資料、再利用できる知識、今回の成果概要 |
...
| 記事を書く | 提供された素材を整理し、必要なら Web で補足して直接執筆する | 使える記事 |
| 継続学習・結果レビュー | 今回の実際の成果物、反応、結果を読む | 次の一課、テーマ、検証可能な改善 |

記事執筆時の Web 補足は、役立つ材料を増やすためのものです。すべての主張を自動で事実確認するわけではありません。調査、情報源の比較、事実確認そのものが必要な場合は、明示的に依頼してください。通常モードでは素材が十分なら記事まで直接進みます。ホストが Plan モードを提供する場合は、深いインタビューにも使えます。Skill は先に自力で取得できる情報を補い、その後で本人にしか答えられない経験、感情、立場、公開範囲を尋ねます。
...
接続後は、原資料、テーマごとの一つの正本、完全な執筆事例、独立した書き出し、確認済みの作品、コンテンツ方向、継続テーマの状態を保存できます。通常の記事執筆では、まず活動中の原資料から同じテーマの字幕、インタビュー、記事を探し、その後で記事事例の索引から一般的な候補を取得します。最終的には、現在の記事に最も適した完全な原文を一つだけ選び、書き方の手本として使います。書き出しは、冒頭の設計や修正を明示的に求められた場合だけ候補になります。テーマ知識、個人の文体、公開履歴を新しい原稿へ自動で持ち込まず、対応する成果を明示的に求めた場合だけ利用します。一度の公開結果が長期戦略や個人の文体を自動で書き換えることもありません。
...
個人ライブラリが未設定でも、素材理解、調査、レビュー、記事執筆は現在の入力から続行できます。公開リポジトリを clone しても個人資料は含まれません。

## 対象範囲

この Skill は、素材理解、テーマ調査、概念説明、実践設計、コンテンツレビュー、記事を扱います。短文、Thread、GitHub プロジェクトの短い紹介や一覧、プロジェクト・製品の短い宣伝文、および画像、GIF、動画、音声、Podcast の制作、一般翻訳、広告運用、セールスページ、メール施策、ブランド全体やマーケティング全体の戦略、実際の公開は別の能力です。
...
├── references/                # 学習、調査、記事執筆、知識ライブラリの方法
