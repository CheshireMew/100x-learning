...
`100x-learning` 是一个遵循 [Agent Skills 开放格式](https://agentskills.io/specification)的学习与文章写作 Skill。它能读取字幕、文章、链接、主题、项目、草稿和发布反馈，再按你要的最终结果选择合适的方法。
...
- **审查或写文章**：`只检查这篇稿子的事实、结构和 AI 味，不要改稿。` 或 `把这些材料写成一篇面向普通读者的中文文章。`
- **连续学习与内容改进**：`围绕 X 连续学习，先完成当前最有用的一课，再根据我的反馈决定下一步。`
- **沉淀知识**：`把确认后的结论保存到我的私人知识库，并更新同主题的现有真源。`

Skill 会停在你指定的结果：审查不会自动改稿，候选不会自动扩写成文章，一次选题不会变成长线项目。保存文件、制作媒体、上传和发布也都是独立动作。
...
| 写成文章 | 整理现有材料，按需联网补充，再直接成文 | 可使用的文章 |
| 持续学习或复盘 | 读取本轮真实产物、反馈和结果 | 下一课、下一题或下一次可验证改进 |

文章写作中的联网补充用于增加有用材料，不等于逐项事实核查；只有你明确要求研究、比较来源或核查事实时，才会把研究本身作为交付结果。普通模式在材料足够时直接完成文章；宿主提供 Plan 模式时，你可以用它进行深度访谈，Skill 会先补齐可查询的信息，再询问只有你能回答的经历、情绪、立场和表达边界。
...
接入后，私人库可以保存来源、同主题知识真源、完整写作案例、独立钩子、确认后的作品、内容方向和持续选题状态。普通文章写作会先从活动原始来源中寻找同主题的字幕、访谈和文章，再按索引取得通用文章案例候选，最终只选一份最契合当前文章的完整原文作为写法范本；只有你明确要求专门设计或修改开头时，钩子才会进入候选。主题知识、作者声音和发布历史不会自动进入新稿，只有你明确要求对应结果时才使用。单次发布表现也不会自动改写长期策略或个人声音。
...
没有配置私人库时，材料理解、研究、审查和文章写作仍然可以直接使用当前输入完成。克隆公开仓库也不会带走任何私人资料。

## 能力边界

这个 Skill 负责理解材料、研究主题、解释概念、设计实践、审查内容和写作文章。短帖、Thread、GitHub 项目短介绍与清单、项目或产品短宣发文案，以及图片、GIF、视频、音频与播客制作、普通翻译、广告投放、销售页、邮件营销、品牌全案、完整营销策略和实际发布，不在它的交付范围内。
