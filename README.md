...
写作任务会分开管理完整案例、独立钩子和个人声音。材料足够只表示不联网补论据，新写仍会读取私人库参考。短内容与文章使用各自的案例索引，短内容、Thread 与文章使用各自的钩子索引；所有索引只按开头、推进和收束等写作技巧分组，条目与文件名使用不带题材含义的稳定编号，不能先看标题再按对象、题材、行业或任务挑参考。模型阅读全文后自由综合，案例与钩子只影响写法，不提供当前对象的事实、身份和声音。没有确认过的用户声音时使用自然、清楚的目标语言；来源作者的第一人称、立场和口吻不会转给用户。
...
| 本身已经是完整成品，并且值得作为写作参考 | 完整短内容或文章案例目录 | 从第一句到最后一句保留完整正文和写作技巧，不附加来源，并进入对应形态的案例索引 |
| 用户明确指定保存的一段原始开头 | `20-Sources/Hook Library/Examples/<写作技巧>` | 保存连续原文、紧接内容和适用形态，不附加来源，并进入对应形态的钩子索引 |
| 用户确认的终稿或已发布作品 | `40-Outputs/Writing` | 保存最终正文，并按真实来源决定是否进入发布历史或作者声音检索 |

一次材料可以产生来源记录、知识增量或完整案例；钩子只由用户明确的钩子保存动作产生。写入完成后，Skill 会重新读取实际真源，并按成品形态与写作技巧重建独立索引，让下一次检索和写作真正读到结果。
