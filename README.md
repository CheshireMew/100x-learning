...
接入后，私人库可以保存来源、同主题知识真源、完整写作案例、独立钩子、确认后的作品、内容方向和持续选题状态。普通写作先从完整材料里选择一个没读过来源的人也能立即理解的具体角度，再从仓库内分开存放的钩子、承接、正文、连接、结尾和长文章节模板中独立选择实际需要的组件，并在成品后列出使用情况；承接或连接没有写就标为未使用。私人库可用时，取例工具先列出候选标题；模型可以为同一组件完整比较多份仍可能相关的案例，再决定是否采用其中一份，不会自动挑选。案例不提供本轮事实。普通写作不会读取同主题知识、作者声音或发布历史；这些资料只有在你明确要求对应任务时才使用。单次发布表现也不会自动改写长期策略或个人声音。
