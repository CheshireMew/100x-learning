...
`SKILL.md` 是唯一总入口。它先根据用户最终要得到的结果选择路径，再按需读取 `references/` 中的对应方法。材料理解、主题研究、写作、持续选题和发布复盘使用不同流程。写作前可以联网发现有助理解、增加信息量或提升传播力的新材料，用户明确要求联网补充时直接执行；这不是逐项验证用户说法的事实核查，也不成为开始写作的前置门槛。只有用户明确要求研究、比较来源或事实核查时才进入研究流程。准备阶段只移开无关内容、完全重复内容、旧成品和示例文案，融资、合作、阶段、数字、邀请条件和其它不同关系不会因为正文较短而被提前删掉。

普通写作从私人库只读取完整案例和独立钩子，不搜索同主题知识、其它来源、项目、成果、作者声音、发布历史或内容策略。独立短帖与 Thread 共用社交内容案例索引，文章使用文章案例索引；短内容、Thread 与文章共用一份钩子索引。每次写作根据当前成品需要的表达作用，从案例索引和钩子索引各选择三个不同的写作技巧分组，再沿稳定编号链接打开完整原文；质量或作用不合适时只在原分组换读另一份。候选发现不会扫描正文目录，也不会拿句式、修辞词组或题材关键词搜索案例。最终默认选入三份彼此不同的完整案例和三份完整钩子，跨题材学习开头、信息推进和收束；只有本地索引无法提供这些写法时，才会临时读取公开完整内容，而且不会自动保存。案例与钩子只影响写法，不提供当前对象的事实、身份和声音；来源作者的第一人称、立场和口吻不会转给用户。最终只展示真正送入成文模型的参考，并附本地绝对路径或公开网页链接。

成文模型只把用户要生成什么及其明确硬约束放进写作要求；用户随请求提供的事实、关系、判断、猜测、问题、角度和内容主次进入材料，并与联网补充的来源材料、多份完整案例、多份完整钩子，以及用户当前提供或明确要求取得的声音材料一起交给模型。文章和 Newsletter 也不会自动读取私人库声音；个人声音、发布历史和查重只在用户明确要求对应结果时使用。专项说明、维护规则、提纲或通用写法清单不会进入成文输入。多份案例之间和多份钩子之间分别使用 `---` 分隔。用户没有禁止联网时，Skill 不会自行把“不搜索外部资料”加入写作要求。每次写作固定展示写作要求、写作准备材料、结果和本次创作参考四部分。
...
默认库根记录在当前用户的 `~/.100x-learning/config.json`。接入后，私人库可以提供知识真源、完整内容案例、钩子、已确认的写作声音和发布历史。普通写作只使用其中的案例和钩子；主题知识服务明确的研究或知识维护，作者声音和发布历史服务用户明确要求的独立结果。没有配置私人库时，材料理解、研究和写作仍可使用当前输入完成，不会把私人库变成普通写作的门槛。
...
| 本身已经是完整成品，并且值得作为写作参考 | 完整社交内容或文章案例目录 | 从第一句到最后一句保留完整正文和写作技巧，不附加来源；短帖和 Thread 进入同一份社交内容案例索引 |
| 用户明确指定保存的一段原始开头 | `20-Sources/Hook Library/Examples/<写作技巧>` | 保存连续原文和紧接内容，不附加来源，也不记录适用形态，进入统一钩子索引 |
| 用户确认的终稿或已发布作品 | `40-Outputs/Writing` | 保存最终正文，并按真实来源决定是否进入发布历史或作者声音检索 |

一次材料可以产生来源记录、知识增量或完整案例；钩子只由用户明确的钩子保存动作产生。写入完成后，Skill 会重新读取实际真源：案例按社交内容或文章以及写作技巧重建索引，钩子只按写作技巧重建统一索引，让下一次检索和写作真正读到结果。
