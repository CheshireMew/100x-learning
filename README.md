...
`SKILL.md` 是唯一总入口。它先根据用户最终要得到的结果选择路径，再按需读取 `references/` 中的对应方法。材料理解、主题研究、写作、持续选题和发布复盘使用不同流程：研究负责弄清事实；所有可发布文字首先服务传播。用户要短内容且给定材料已经足够时，直接用这些材料成文，不为增加“洞察”或“传播力”补搜外部论据；只有材料缺少关键内容或用户要求研究核实时才进入发现式新写，并且不重新确认用户已经给出的数字、日期、条件或结论。草稿完成后才核对写作者新增或外部补充材料带来的主张。复盘则从真实成品和反馈中形成下一次可验证的改进。

写作任务会分开管理完整案例、独立钩子和个人声音。材料足够只表示不联网补论据，新写仍会读取私人库参考。独立短帖与 Thread 共用社交内容案例索引，文章使用文章案例索引；短内容、Thread 与文章共用一份钩子索引。所有索引只按开头、推进和收束等写作技巧分组，条目与文件名使用不带题材含义的稳定编号，不能先看标题再按对象、题材、行业、任务或成品形式挑参考。模型阅读全文后自由综合，案例与钩子只影响写法，不提供当前对象的事实、身份和声音。没有确认过的用户声音时使用自然、清楚的目标语言；来源作者的第一人称、立场和口吻不会转给用户。
...
默认库根记录在当前用户的 `~/.100x-learning/config.json`。接入后，私人库可以提供知识真源、完整内容案例、钩子、已确认的写作声音和发布历史。没有配置私人库时，材料理解和独立研究仍可使用当前输入完成；材料完备短内容和发现式新写都会在外部材料发现前停止，并明确告诉用户先初始化或接入私人库。明确属于用户本人现稿的字词、格式或等义压缩继续按局部修改完成。
