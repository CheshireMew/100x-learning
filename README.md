...
接入后，私人库可以保存来源、同主题知识真源、完整写作案例、独立钩子、确认后的作品、内容方向和持续选题状态。普通写作会从仓库内分开存放的钩子、承接、正文、连接、结尾和长文章节模板中独立选择，并在成品后列出实际使用的模板；私人库可用时，取例工具会为钩子、正文、结尾和实际使用的长文章节各挑一份完整案例，帮助自然表达。案例不提供本轮事实。普通写作不会读取同主题知识、作者声音或发布历史；这些资料只有在你明确要求对应任务时才使用。单次发布表现也不会自动改写长期策略或个人声音。
