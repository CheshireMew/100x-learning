...
| 设计内容型宣发 | `为这个项目设计发布期宣发，先给方案。` | 先锁定读者、利益、可写材料、时间和入口；确认后再写 |
...
写作任务还会分开管理完整案例、开头技巧和个人声音。案例提供写法，不替当前对象提供能力、数字或承诺；内容型宣发则会先把利益领取者、触发动作、有效时间和真实入口固定为发布要求，再在成稿后按正文实际承诺核查。
...
```text
阅读这份材料并沉淀到我的私人知识库。保留值得长期复用的知识；如果它本身是完整成品，再判断是否进入内容案例；只有开头方法可以脱离原题材复用时，才建立钩子。
```

写入只在用户明确要求时发生。原始材料、稳定知识、完整案例和钩子可以服务不同用途，但完整原文只保留一份真源：知识文档链接来源，钩子引用完整案例，不复制正文。
...
| 本身已经是完整成品，并且值得作为写作参考 | 完整短内容或文章案例目录 | 从第一句到最后一句保留完整正文和可核对来源，并进入统一案例索引 |
| 可以脱离原题材复用的开头动作 | `钩子与开头` | 保存技巧、预期读者反应和来源案例引用，不复制完整正文 |
| 用户确认的终稿或已发布作品 | `40-Outputs/Writing` | 保存最终正文，并按真实来源决定是否进入发布历史或作者声音检索 |

一次材料可以同时产生来源记录、知识增量、案例资格和钩子资格，但这些身份不会制造多份全文。写入完成后，Skill 会重新读取实际真源并重建相应索引，让下一次知识检索、案例浏览或写作记忆真正读到结果。
...
测试覆盖的是完整生产与消费链：新库会从空目录初始化，再由独立进程通过本机指针重新定位和验证；真实 UTF-8 材料会经过 `add-case` 建立完整案例，钩子通过 `add-hook` 引用该案例，统一索引随后由正式读取入口消费；符合写作来源条件的案例还会继续进入写作记忆检索。测试不会由消费端手写一份假索引来冒充生产者结果。
