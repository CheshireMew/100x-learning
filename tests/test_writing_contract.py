...
class SkillStructureTests(unittest.TestCase):
    ...
    def test_normal_writing_selects_independent_components(self) -> None:
        ...
        self.assertIn("先选最强角度，再分开选择写作部件", skill)
        self.assertIn("先从当前材料中选出最值得读者停下来的具体角度", index)
        ...
        self.assertIn("模板管职责，相关例子才帮助表达", skill)
        self.assertIn("从最终文字反向确认组件", skill)
        self.assertIn("不能混用两种正文推进却只标一个模板", skill)
    ...
    def test_writer_input_has_material_components_and_optional_voice(self) -> None:
        ...
        self.assertIn("没有相关例子时直接不用", content)
    ...
    def test_workflow_components_are_independent(self) -> None:
        ...
        self.assertIn("这组输入与输出是当前材料中最能说明对象价值的具体细节", hook)
        self.assertIn("项目介绍", body)
        self.assertIn("这条链路能直接说明正文的主要对象或价值", body)
        self.assertIn("可见输出或结果", continuation)
        self.assertIn("输入（前文已经写出时不复述）→ 可见输出与中间动作 → 能力证据", body)
        self.assertIn("一至两组直接支持这条链路的能力证据", body)
        self.assertIn("同类资源的多个数量算一组", body)
    ...
    def test_readmes_describe_template_runtime(self) -> None:
        ...
        self.assertIn("先列出候选标题，再由模型按当前角度决定是否读取一份完整案例", readme)
