...
class SkillStructureTests(unittest.TestCase):
    ...
    def test_normal_writing_selects_independent_components(self) -> None:
        ...
        self.assertIn("先让读者看懂，再分开选择写作部件", skill)
        self.assertIn("目标读者无需预先读过来源也能看懂", index)
        self.assertIn("组件不预先绑定成整篇路线", skill)
        self.assertIn("组件独立选择，不使用整篇模板、固定套餐、项目类型默认路线或推荐组合", content)
        ...
        self.assertIn("模板管职责，完整例子帮助表达", skill)
        self.assertIn("从最终文字反向确认组件", skill)
        self.assertIn("正文可以加入帮助读者理解主线的必要解释", skill)
        self.assertIn("清单根据核对后的最终文字生成", content)
    ...
    def test_old_case_and_hook_mixing_runtime_is_gone(self) -> None:
        ...
        self.assertIn("先运行 `scripts/select_writing_examples.py candidates`", skill)
        self.assertIn("禁止由脚本按固定排序自动选例", skill)
    ...
    def test_writer_input_has_material_components_and_optional_voice(self) -> None:
        content = _read("references/content-writing.md")
        for heading in ("【用户要求】", "【材料】", "【当前角度】", "【结构组件】", "【模仿例子】", "【作者声音】"):
        ...
        self.assertIn("模仿例子可以影响读者怎样进入内容", content)
        self.assertIn("陌生说法怎样落到自然中文", content)
        self.assertIn("人物、对象、数字、观点、情绪和行动入口不能进入本轮成品", content)
        self.assertIn("不相关时一份也不采用", content)
    ...
    def test_template_selection_rejects_missing_facts(self) -> None:
        ...
        self.assertIn("`H`、`P` 或 `E` 没有合格组件时不成文", skill)
    ...
    def test_workflow_components_are_independent(self) -> None:
        ...
        self.assertIn("项目介绍", hook)
        self.assertIn("目标读者即使不熟悉其中语法", hook)
        self.assertIn("项目介绍", body)
        self.assertIn("这条链路能直接说明正文的主要对象或价值", body)
        self.assertIn("可见输出或结果", continuation)
        self.assertIn("输入（前文已经写出时不复述）→ 可见输出与中间动作 → 必要时补直接证据", body)
        self.assertIn("帮助目标读者理解这条链路价值的能力证据", body)
        self.assertIn("不列工具名、规格和性能数字证明项目很强", body)
        self.assertIn("两者先后服从自然中文", body)
        self.assertIn("命令主要由陌生英文名、参数或符号组成", hook)
        self.assertIn("只用“配置好后”等自然说法交代准备已经完成", hook)
        self.assertIn("不在开头展开安装步骤或平台兼容表", hook)
        self.assertIn("单一来源或入口", ending)

    def test_project_promotion_chooses_the_strongest_detail_before_components(self) -> None:
        index = _read("references/writing-templates/index.md")
        ...
        self.assertIn("先找出一个目标读者能够直接理解的角度", index)
        self.assertIn("项目类别、内容形式和模板编号都不能代替这一步", index)
        self.assertIn("真实输入或命令只有不熟悉语法的目标读者也能从中看出用途和结果时才承担开头", project)
        self.assertIn("用户没有指定开发背景时，不假定读者已经理解依赖、虚拟环境、锁文件、缓存、CLI", project)
        self.assertIn("“依赖”写成项目需要的软件包", project)
        self.assertIn("项目统一替代多套旧工具时，直接写过去要在多个工具之间切换", project)
        self.assertIn("不能把一串功能名或工具名当作项目价值", project)
        self.assertIn("不保留实现语言、命令名、竞品名单或操作系统名单", project)
        self.assertIn("草稿完成后只留一个主要结果和一项最强证据", project)
        self.assertIn("不先补一个较弱的中间产物再否定它制造反差", project)
        self.assertIn("短帖可以用“配置好后”概括必要设置已经完成", project)
        self.assertIn("命令、数字、专门说法或概括性类别词只有在目标读者无需额外背景也能理解", content)
        self.assertNotIn("项目介绍或宣传短帖优先用 `H01`", contract)
        self.assertNotIn("`H23` 留给操作演示", contract)
        self.assertIn("即使它同时属于正文，也要列为已使用", index)
        self.assertIn("若只是直接执行 `P`", content)
        self.assertIn("一至三项能让目标读者更理解这个结果", evidence_body)
        self.assertIn("不复述前文已经写出的结果、利益、命令或输出", evidence_body)
        self.assertIn("同一谓语下的两项同类事实合成一句", evidence_body)
        self.assertIn("需要先解释术语才能成立的弱证据直接舍弃", evidence_body)

    def test_short_writing_is_understandable_without_source_jargon(self) -> None:
        skill = _read("SKILL.md")
        content = _read("references/content-writing.md")
        benefit_hook = _read("references/writing-templates/hooks/H01.md")
        number_hook = _read("references/writing-templates/hooks/H16.md")
        update_body = _read("references/writing-templates/bodies/P03.md")
        scope_continuation = _read("references/writing-templates/continuations/C10.md")
        visible_ending = _read("references/writing-templates/endings/E01.md")
        open_ending = _read("references/writing-templates/endings/E18.md")
        self.assertIn("对题目感兴趣的普通读者", skill)
        self.assertIn("不假定他读过来源、从事相关行业、会编程", content)
        self.assertIn("不用类别词概括开头", benefit_hook)
        self.assertIn("钩子只保留一个能独立成立的数字", number_hook)
        self.assertIn("不把读者写成“经常被绕晕、受够了、一直头疼”等既定状态", benefit_hook)
        self.assertIn("不需要先理解陌生统计对象或专业术语", number_hook)
        self.assertIn("陌生数字和术语不能代替直接影响", update_body)
        self.assertIn("短帖中的必要边界跟随相关主张自然出现", scope_continuation)
        self.assertIn("不追加提醒、限制或下一步", visible_ending)
        self.assertIn("不能从更宽泛的主题自行推导一个大问题", open_ending)
        self.assertIn("不靠结尾新引入术语、对象或另一条知识线", open_ending)
        self.assertIn("只有用户明确要求辟谣或纠正常见误解时", content)
        self.assertIn("不使用“不是、并非或不只是某事，再用而是、却或真正转到重点”的句段", content)
        self.assertIn("来源中的免责声明只约束结论，不能直接复制成这种句式", content)
        self.assertIn("完整例子如果比当前目标读者需要更多专业背景，不能作为本轮模仿对象", content)
        self.assertIn("主张已经把适用对象和范围写清时，不另起提醒句", content)
        self.assertIn("`E01` 后不能再有提醒或限制", content)
        self.assertIn("`E18` 的未知必须由来源明确提出并已在正文展开", content)
        self.assertIn("短内容不采用“不是、并非或不只是某事，再用而是、却或真正转到重点”的纠偏结构", skill)
        self.assertIn("同一句若堆了两个以上目标读者不熟悉的命令、英文名或专业术语", skill)
        self.assertIn("同一句出现两个以上目标读者不熟悉的命令、英文名或专业术语", content)
    ...
    def test_long_writing_constrains_each_section(self) -> None:
        ...
        self.assertIn("长文为每个实际章节各选一个 `S`", index)
        self.assertIn("文章与 Newsletter 的每个实际章节使用一个 `S`", content)
    ...
    def test_case_and_hook_libraries_are_research_corpus_and_scoped_examples(self) -> None:
        ...
        self.assertIn("正式取例工具列为钩子、正文、结尾或长文章节的模仿候选", cases)
        self.assertIn("独立钩子与承接组件的蒸馏语料", hooks)
        self.assertIn("正式取例工具先返回当前钩子组件的候选标题", hooks)
    ...
    def test_writing_completes_in_one_turn_without_handoff(self) -> None:
        ...
        self.assertIn("材料准备、选角度、选模、取例和成文在同一次回复中连续完成", skill)
    ...
    def test_default_delivery_returns_result_and_used_template_ids(self) -> None:
        ...
        self.assertIn("实际使用的写 ID 与名称，没有使用的写“未使用”", content)
    ...
    def test_readmes_describe_template_runtime(self) -> None:
        ...
        self.assertIn("模型可以为同一组件完整比较多份仍可能相关的案例", readme)
        self.assertNotIn("不读取私人案例、钩子", readme)
        self.assertIn("independently selects only the version-controlled hook, continuation, body, bridge, ending", readme_en)
        self.assertIn("lists candidate titles first", readme_en)
        self.assertIn("compare several still-relevant complete cases for the same component", readme_en)
        self.assertNotIn("does not read private cases, hooks", readme_en)
        self.assertIn("フック、承接、本文、接続、結び、長文セクションから実際に必要なものだけを選び", readme_ja)
        self.assertIn("候補タイトルを先に一覧表示", readme_ja)
        self.assertIn("同じコンポーネントについて、なお関連しそうな完全な事例を複数読んで比較", readme_ja)
